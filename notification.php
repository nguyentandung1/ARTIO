...
} elseif ($_SESSION["typeOfUser"] == "seller") {
...
foreach ($Notifications as $noti) {
    if ($noti['seen'] == 0) {
        $_SESSION['noOfNewNotification']++;
    } else {
        $_SESSION['noOfOldNotification']++;
    }
}
...
<?php 
  ...
  if(isset($_SESSION["typeOfUser"]) && $_SESSION['typeOfUser']=='buyer')
    setNotificationsSeenForBuyer($db,$_SESSION['id']);
  elseif(isset($_SESSION["typeOfUser"]) && $_SESSION['typeOfUser']=='seller')
    setNotificationsSeenForSeller($db,$_SESSION['id']);
  ob_end_flush(); 
?>