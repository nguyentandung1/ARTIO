...
function UpdateItemCount($cartId, $quantity, $itemId, $db) {
    if (isset($userId) && isset($quantity) && isset($itemId)) {
...
function CheckBuyerAndItem($cartId, $itemId, $db): mixed {
...
function getFeaturedItems($db, $limit = 10) {
    $stmt = $db->prepare("
        SELECT item.*,
               itemimage.image,
               CONCAT(seller.fName, ' ', seller.lName) AS sellerName
        FROM item
        LEFT JOIN itemimage ON item.itemId = itemimage.itemId
        LEFT JOIN seller ON item.sellerId = seller.ID
        ORDER BY item.addDate DESC
        LIMIT ?
    ");
...
function getLatestItems($db, $limit = 10) {
    $stmt = $db->prepare("
        SELECT item.*,
               itemimage.image,
               CONCAT(seller.fName, ' ', seller.lName) AS sellerName
...
function hasPendingAuctionRequest($db, $itemId, $sellerId) {
    $stmt = $db->prepare("
        SELECT COUNT(*) 
...
function addAuctionRequest($db, $itemId, $sellerId) {
    $stmt = $db->prepare("
        INSERT INTO auction_requests(itemId, sellerId, status, createdAt)
        VALUES (?, ?, 'pending', NOW())
    ");
    return $stmt->execute([$itemId, $sellerId]);
}
