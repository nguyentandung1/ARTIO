...
if(isset($_GET['auction_item_id'])){
    $itemId = $_GET['auction_item_id'];
    $sellerId = $_SESSION['id'];

    if(!hasPendingAuctionRequest($db, $itemId, $sellerId)){
        addAuctionRequest($db, $itemId, $sellerId);
    }
          ...
          <?php echo '       
                    <div class="card-body">
                                ...
                                <div class="card-body">
                                  ...
                                  <a href="profileSeller.php?auction_item_id=' . $forSaleItem->itemId . '" 
                                    class="btn btn-warning">
                                    Auction
                                  </a>

                                  <a href="profileSeller.php?delete_id=' . $forSaleItem->itemId . '" 
    ...
    <div class="col-sm-12">
      ...
      <section class="row flex-row flex-nowrap p-3 overflow-auto profile_scroll rounded position-static "
        ...
        <?php
                    ...
                    $childcategory = getchildcategory($forSaleItem->childcategoryId, $db)[0];
                    $countOrders = getPendingOrdersCount($forSaleItem->itemId,$db);
                    $auctionCheck = $db->prepare("
                        SELECT status
                        FROM auction
                        WHERE itemId = ?
                        AND status IN ('pending','approved','ended')
                        LIMIT 1
                    ");
                    $auctionCheck->execute([$forSaleItem->itemId]);
                    $auctionStatus = $auctionCheck->fetch();
                    echo '
                    <div class="col-lg-3 m-0 text-center">
                        <div class="card m-md-auto shadow" style="width: 18rem;">
                                '; ?>
