...
function GetChildCategories($db) {
  $sql = "SELECT CC.childcategoryId,CC.childcategoryName, C.categoryName,CC.childcategoryDescription
          FROM childcategory as CC,category as C
...
function AddNewChildCategory($name, $des,$catId, $db) {
  $insertSql = "INSERT INTO childcategory (childcategoryName, categoryId, childcategoryDescription) VALUES ('" . $name . "','" . $catId . "', '" . $des . "');";
  $db->exec($insertSql);
}
...
function GetChildCategoryByID($id, $db) {
  $sql = "SELECT * FROM childcategory WHERE childcategoryId=" . $id . ";";
  $stmt = $db->query($sql);
  $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
  return $result;
}
...
function UpdateChildCategory($id, $name, $catId, $des, $db) {
  $updateSql = "UPDATE childcategory SET childcategoryName='" . $name . "', childcategoryDescription='" . $des . "', categoryId='" . $catId . "' WHERE childcategoryId=" . $id . ";";
  $db->exec($updateSql);
}
...
function DeleteChildCategoryByID($id, $db) {
  $deleteSql = "DELETE FROM childcategory WHERE childcategoryId=" . $id . ";";
  $db->exec($deleteSql);
}
//End Categories
...
function GetItems($db) {
  $sql = "SELECT I.itemId,I.title,CC.childcategoryName,S.fName,S.lName,I.price,I.quantity 
          FROM item as I,childcategory as CC,seller as S 
          WHERE I.childcategoryId=CC.childcategoryId AND I.sellerId=S.ID;";
  $stmt = $db->query($sql);
  $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
  return $result;
}
...
function ApproveSellerRequest($requestId, $minPrice, $startTime, $endTime, $db) {
    ...
    if($request){
        ...
        $stmt2 = $db->prepare("
            INSERT INTO auction
            (itemId, sellerId, minPrice, startTime, endTime, status)
            VALUES (?, ?, ?, ?, ?, 'approved')
        ");
        ...
        $stmt3 = $db->prepare("
            ...
            WHERE requestId = ?
        ");
        $stmt3->execute(array($requestId));
    }
}

function RejectSellerRequest($requestId, $db) {
    $stmt = $db->prepare("
        UPDATE auction_requests
        SET status = 'rejected'
        WHERE requestId = ?
